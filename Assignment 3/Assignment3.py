...
def main():
	...
	processed_img1 = cv2.filter2D(grayscale, -1, kernel_1)
	processed_img2 = cv2.filter2D(grayscale, -1, kernel_2)
	processed_img3 = cv2.filter2D(grayscale, -1, kernel_3)
	processed_img4 = cv2.filter2D(grayscale, -1, kernel_4)
	processed_img5 = cv2.filter2D(grayscale, -1, kernel_5)
	processed_img6 = cv2.filter2D(grayscale, -1, kernel_6)
...
def PlotImg(img_set, title_set):
    ...
    for i in range(1):
        ...
        plt.title(title_set[7])
        plt.savefig('Kernel 6.jpg')
    plt.show()
